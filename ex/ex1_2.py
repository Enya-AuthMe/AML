...
def printing_Kfold_scores(x_train_data, y_train_data): 
    ...
    svc = svm.SVC(probability=True, kernel='rbf', C=2250, gamma=0.002)
    # nusvc = svm.NuSVC(probability=True)
    model = CalibratedClassifierCV(svc, cv=5, method='sigmoid')
    model.fit(x_train_data.values, y_train_data.values.ravel())
    y_pred = model.predict(x_train_data.values)
    y_proba = model.predict_proba(x_train_data.values)
    recall = recall_score(y_train_data.values, y_pred)
    precision = precision_score(y_train_data.values, y_pred)
    f1 = f1_score(y_train_data.values, y_pred)
    evaluation = contest_eval(y_pred, y_proba, y_train_data.values.ravel())
    print('--- Training ---')
...
# Plot SVM classifier
disp = DecisionBoundaryDisplay.from_estimator(trained_model, X_test, response_method='auto', xlabel=Y_test)
breakpoint()
